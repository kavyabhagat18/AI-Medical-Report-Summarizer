...
# ---------------------------------------------------------------
# SAFETY NET: common real-world medical codes/abbreviations that are
# made ENTIRELY of characters from the OCR-lookalike set (0-9, O, o,
# I, l, S, B) plus at least one real digit -- meaning the digit
# correction logic below would treat them as a mangled number and
# silently corrupt them (e.g. "Vitamin B12" -> "Vitamin 812").
# These are protected before normalization runs, and restored after.
# Add to this list if another vulnerable code is found in the wild.
# ---------------------------------------------------------------
PROTECTED_MEDICAL_TERMS = ["B1", "B2", "B6", "B9", "B12", "O2", "S100", "B27"]
_PROTECTED_TERM_PATTERN = re.compile(
    r"\b(" + "|".join(re.escape(t) for t in PROTECTED_MEDICAL_TERMS) + r")\b",
    re.IGNORECASE,
)


def _protect_medical_terms(text: str) -> tuple[str, dict]:
    """Temporarily replace protected terms with placeholders that won't match any digit-lookalike pattern."""
    placeholders: dict = {}

    def _replace(match: re.Match) -> str:
        key = f"QPTKNQ{len(placeholders)}Q"
        placeholders[key] = match.group(0)
        return key

    return _PROTECTED_TERM_PATTERN.sub(_replace, text), placeholders


def _restore_medical_terms(text: str, placeholders: dict) -> str:
    """Restore the original protected terms after all normalization steps have run."""
...
def apply_regex(text: Optional[str]) -> str:
    ...
    try:
        # Step 0: Protect medical codes that would otherwise be corrupted by
        # digit-lookalike correction below (e.g. "Vitamin B12" -> "Vitamin 812")
        text, protected_placeholders = _protect_medical_terms(text)
        ...
        # Step 13: Restore protected medical terms now that digit-lookalike
        # correction has finished running.
        text = _restore_medical_terms(text, protected_placeholders)
        
    except Exception as e:
        logger.error(f"Unexpected error in apply_regex: {e}")
        # Still attempt to restore protected terms even if a step above failed,
        # so a mid-pipeline error doesn't leave placeholder tokens in the text.
        if 'protected_placeholders' in locals():
            text = _restore_medical_terms(text, protected_placeholders)
